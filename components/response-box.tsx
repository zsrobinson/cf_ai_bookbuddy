...
export function ResponseBox({ children }: { children: ReactNode }) {
  return (
    <div className="border border-cyan-500/50 rounded-xl p-4 flex gap-4 items-center bg-gradient-to-tr from-blue-500/10 to-emerald-500/10 ">
      ...
      <p className="text-cyan-900 dark:text-cyan-100">{children}</p>
    </div>
  );
}

export function AnimatedResponseBox({ text }: { text: string }) {
